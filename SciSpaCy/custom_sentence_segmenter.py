def combined_rule_splitter(doc):
    ...
    for token in doc:
        ...
        if token.text[0].isdigit():
            ...
            if prev_tokens[-1] and prev_tokens[-1].text in ["sec.", "secs.", "Sec.", "Secs.", "fig.", "figs.", "Fig.", "Figs.", "eq.", "eqs.", "Eq.", "Eqs.", "no.", "nos.", "No.", "Nos."]:
                doc[token.i].is_sent_start = False
        ...
        # sentences cannot start right after a )
        if prev_tokens[-1] and prev_tokens[-1].text == ")":
            doc[token.i].is_sent_start = False
        ...
        # attempt to handle double and quadruple new lines around section headers
