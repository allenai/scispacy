...
def test_tokenizer_handles_no_word(combined_rule_tokenizer_fixture):
    tokens = combined_rule_tokenizer_fixture("")
    assert len(tokens) == 0


@pytest.mark.parametrize('text', ["lorem"])
def test_tokenizer_handles_single_word(combined_rule_tokenizer_fixture, text):
    tokens = combined_rule_tokenizer_fixture(text)
    assert tokens[0].text == text


def test_tokenizer_handles_punct(combined_rule_tokenizer_fixture):
    text = "Lorem, ipsum."
    tokens = combined_rule_tokenizer_fixture(text)
    assert len(tokens) == 4
...
def test_tokenizer_handles_digits(combined_rule_tokenizer_fixture):
    exceptions = ["hu", "bn"]
...
def test_tokenizer_keep_urls(combined_rule_tokenizer_fixture, text):
    tokens = combined_rule_tokenizer_fixture(text)
    assert len(tokens) == 1


@pytest.mark.parametrize('text', ["NASDAQ:GOOG"])
def test_tokenizer_colons(combined_rule_tokenizer_fixture, text):
    tokens = combined_rule_tokenizer_fixture(text)
    assert len(tokens) == 3


@pytest.mark.parametrize('text', ["hello123@example.com", "hi+there@gmail.it", "matt@explosion.ai"])
def test_tokenizer_keeps_email(combined_rule_tokenizer_fixture, text):
    tokens = combined_rule_tokenizer_fixture(text)
    assert len(tokens) == 1


def test_tokenizer_handles_long_text(combined_rule_tokenizer_fixture):
...
def test_tokenizer_handle_text_from_file(combined_rule_tokenizer_fixture, file_name):
...
def test_tokenizer_suspected_freeing_strings(combined_rule_tokenizer_fixture):
    text1 = "Lorem dolor sit amet, consectetur adipiscing elit."
    text2 = "Lorem ipsum dolor sit amet, consectetur adipiscing elit."
    tokens1 = combined_rule_tokenizer_fixture(text1)
    tokens2 = combined_rule_tokenizer_fixture(text2)
...
def test_tokenizer_add_special_case(combined_rule_tokenizer_fixture, text, tokens):
    combined_rule_tokenizer_fixture.add_special_case(text, tokens)
