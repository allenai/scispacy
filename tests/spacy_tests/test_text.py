...
def test_en_tokenizer_handles_long_text(combined_rule_tokenizer):
...
def test_en_tokenizer_handles_cnts(combined_rule_tokenizer, text, length):
    tokens = en_tokenizer(text)
...
def test_lex_attrs_like_number(combined_rule_tokenizer, text, match):
    tokens = en_tokenizer(text)
    assert len(tokens) == 1
