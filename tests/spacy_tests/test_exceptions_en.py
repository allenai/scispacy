...
def test_en_tokenizer_handles_basic_contraction(combined_rule_tokenizer):
    text = "don't giggle"
...
def test_en_tokenizer_handles_basic_contraction_punct(combined_rule_tokenizer, text):
    tokens = en_tokenizer(text)
    assert len(tokens) == 3


@pytest.mark.parametrize('text_poss,text', [("Robin's", "Robin"), ("Alexis's", "Alexis")])
def test_en_tokenizer_handles_poss_contraction(combined_rule_tokenizer, text_poss, text):
...
def test_en_tokenizer_splits_trailing_apos(combined_rule_tokenizer, text):
    tokens = en_tokenizer(text)
    assert len(tokens) == 2
...
def text_tokenizer_doesnt_split_apos_exc(combined_rule_tokenizer, text):
    tokens = en_tokenizer(text)
    assert len(tokens) == 1
    assert tokens[0].text == text
...
def test_en_tokenizer_handles_ll_contraction(combined_rule_tokenizer, text):
    tokens = en_tokenizer(text)
    assert len(tokens) == 2
    assert tokens[0].text == text.split("'")[0]
...
def test_en_tokenizer_handles_capitalization(combined_rule_tokenizer, text_lower, text_title):
...
def test_en_tokenizer_keeps_title_case(combined_rule_tokenizer, pron, contraction):
...
def test_en_tokenizer_excludes_ambiguous(combined_rule_tokenizer, exc):
    tokens = en_tokenizer(exc)
    assert len(tokens) == 1


@pytest.mark.parametrize('wo_punct,w_punct', [("We've", "``We've"), ("couldn't", "couldn't)")])
def test_en_tokenizer_splits_defined_punct(combined_rule_tokenizer, wo_punct, w_punct):
...
def test_en_tokenizer_handles_abbr(combined_rule_tokenizer, text):
    tokens = en_tokenizer(text)
    assert len(tokens) == 1


def test_en_tokenizer_handles_exc_in_text(combined_rule_tokenizer):
...
def test_en_tokenizer_handles_times(combined_rule_tokenizer, text):
    tokens = en_tokenizer(text)
    assert len(tokens) == 2
...
def test_en_tokenizer_norm_exceptions(combined_rule_tokenizer, text, norms):
    tokens = en_tokenizer(text)
    assert [token.norm_ for token in tokens] == norms


@pytest.mark.parametrize('text,norm', [("radicalised", "radicalized"), ("cuz", "because")])
def test_en_lex_attrs_norm_exceptions(combined_rule_tokenizer, text, norm):
    tokens = en_tokenizer(text)
